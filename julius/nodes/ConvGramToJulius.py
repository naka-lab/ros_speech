...
def ToHiragana( str ):
    # for windows
    #p = os.popen( "KanjiConv.exe " + str.encode("utf8") , "r" )
    #p = os.popen( "echo %s | kakasi -JH -i utf8 -o utf8 | kakasi -KH -i utf8 -o utf8" % str.encode("utf8") , "r" )
    #hiragana = p.read().decode("utf8").replace("\n","").replace("\r","")
    #p.close()
    hiragana = str
    return hiragana
    ...
    for line in lines:
        ...
        if start:
            ...
            if ":" in line:
                ...
                utt = utt.strip()
                # 念のためnounも正規化
                for nounid in nounIdFinder.findall( utt ):
                    utt = utt.replace( nounid , "$noun" )
                utterances.append( (id,utt) )
    """


    return grammars,nouns,idDict,utterances

def SaveJuliusGram( grammars , nouns, idDict, utterances, basename ):

    nounClassUsedInGram = []
    f = codecs.open( basename + ".grammar" , "w" , "utf8" )
    for g in grammars:
        f.write( "S : " + " ".join(g[2]) + "\n" )
        for w in g[2]:
            if w.find("noun_")==0:
    ...
    f = codecs.open( basename + ".voca" , "w" , "utf8" )
    ...
    """
    # nounのidと結びつけを保存
    f = codecs.open( basename + ".nounid" , "w" , "utf8" )
    for nounClass in nouns.keys():
        for nounInfo in nouns[nounClass]:
            f.write( nounClass + "." + nounInfo[1] + "\t" + nounInfo[0] + "\n" )
        f.write("/n")
    f.close()

    # 発話を保存
    f = codecs.open( basename + ".utter" , "w" , "utf8" )
    for id,utt in utterances:
        f.write(id + "\t" + utt + "\n")
    f.close()
    """
...
def CompileGrammar( txtgram, juliusgram ):
    ...
    grammars,nouns,idDict,utterances = LoadGram(txtgram)
    SaveJuliusGram( grammars , nouns, idDict, utterances , juliusgram )
...
def main():
    ...
    # nouns : { 単語クラス : [(単語ID, 単語文字列, 音素列),...],  ... }
    # idDict : { 単語文字列 : (単語ID, 音素表記) }
    grammars,nouns,idDict,utterances = LoadGram(sys.argv[1])
    SaveJuliusGram( grammars , nouns, idDict, utterances , sys.argv[2] )
    
    nounidDict = {}
    for nounClass in nouns.keys():
        for nounInfo in nouns[nounClass]:
            nounidDict[(nounClass,nounInfo[1])] = nounInfo[0]
    print nounidDict
